...
> **Measured status refresh 2026-09-20 (P2 core-yulu re-key attempt, task 048):** the corpus carries **113** provenance-note strings across four keys (`cbeta_note` 22, `coverage_note` 36, `editorial_note` 41, `recension_note` 14), **77 of 113** rendering beside a passage (the 36 `coverage_note` lines stay ledgered by exemption), **29** documents carrying at least one label (unchanged — `dazhu_huihai` and `nanquan_yulu` already carried labels) — task 048 re-measures the R-A re-key attempts from the recorded witnesses on the 40-work pinned set (refs 40 verified / 0 drift), both below the 80-percent threshold (0 of 6 fields EXACT in any of the 40 refs for dazhu; 0 of 6 in the claimed X68n1315 for nanquan, whose only verbatim carrier of any field is the uncited T47n1997 at s2.d0 27/27), so the `coverage_note`s are rewritten to record the attempts and their measured outcomes, 12 `editorial_note` labels land (6 per document), and no source text is re-keyed.
