...
> - **Phase 2 — `11/14 complete`** · denominator: the 14 documents in `data/corpus/` measured against this phase's own target, *complete* canonical ingestion. `python3 -c "import json;m=json.load(open('data/project_metrics.json'))['corpus'];print(m['documents'],len(m['complete_documents']),m['excerpt_seed_documents'])"` → `14 11 3`: the re-keyed Gateless Gate (2026-09-21) plus the ten documents marked on 2026-09-22 by task 058 (`congronglu`, `chuandenglu_full`, `caoshan_benji`, `huangbo_fayao_full`, `mazu_guanglu_full`, `yunmen_guanglu_full`, `dongshan_yulu_full`, `zhaozhou_yulu_full`, `dahui_yulu_full`, `linji_yulu`) are `complete_selected_witness` — each `collated_to_claimed_witness`, 0 flagged, unit target equal to the units present, and re-verified as a verbatim contiguous tiling of its pinned witness region ([`sessions/P1_COMPLETE_MARKING_2026-09-22.md`](./sessions/P1_COMPLETE_MARKING_2026-09-22.md)); 3 are excerpt seeds (`zhengdao_ge`, `hanshan_poems`, `niutou_juezhu`). "Complete" is scoped to the pinned extraction rule (`tei:note` apparatus dropped) and to the region each `coverage_note` declares; front/back matter outside that region is disclosed, not claimed. The representation targets inside the box are a different claim from completion and were met before the marking: 48/48 *Wumenguan* cases, 100/100 *Congronglu* case records, 14/14 manifest items, 4092/4092 declared case locators (4 document-level seeds remain; the pre-purge figures 100/100 *Biyanlu*, 36/36 items, 248/248 locators described the 2026-09-13 tree).
...
>
> **Attribution-integrity milestone (historical, 2026-08-08)**: provenance policy v2.2 is live (`data/translations/provenance.json`, explicit Reader/Matrix badges ✅/⚠️/🤖); **1 verified corpus quotation slots across 1 texts + 2 verified Matrix entries** (the Wumenguan quotation set covered 48/48 represented cases). Every verified source resolves through `rights_manifest.json`; this English quotation layer is separate from the current W1 source-collation status and rights decisions.
...
│   ├── corpus/                 # 14 active structured source files — 11 complete selected witnesses (marked 2026-09-22, task 058) / 0 partial / 3 excerpt seeds
