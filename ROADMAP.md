...
- [x] **Data Contract & Release Guardrails**:
  ...
  - `data/lineage/lineage_verification.json` aggregates all 31 in-set graph links and 4 unprofiled frontiers with source-record status; `data/lineage/profile_review_queue.json` tracks all 35 profiles (1 in review, 3 complete — the P3 frontier final located the prajnatara, longtan_chongxin and yangqi_fanghui cases with reviewed locators, 30 awaiting exact locators, 1 frontier-source task: dahong_zuzheng, whose link to Yuelin Shiguan is a documented intentional frontier pending exact locator). The public chart never renders a link as source-verified until exact locators are reviewed.
