...
> - **Phase 4 — `5/35 re-keyed`** · denominator: corpus documents. Five documents were re-keyed to a pinned, digest-verified CBETA witness or labelled against one (PRs #29, #30, #32, #34, #35 — see the Wave 1 section below); all **35** were independently inventoried against the pinned witness set; **23** documents carry provenance labels; **593 of 924** source-content fields collate. The presentation gap this line named in earlier revisions is **closed (PR #40, 2026-09-12)**: of the corpus's **50** provenance-note strings, the reader renders **39** — the three passage-level keys at **17** shared-renderer call sites — while the **11** `coverage_note` strings stay in the dossier's represented-units ledger by a recorded, reason-carrying exemption, and a data-driven rule fails on any future orphan key (Phase 4 detail below; implemented and gate-guarded, not browser-verified).
