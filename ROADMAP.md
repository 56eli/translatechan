...
>
> **630 is the register; 532 is today's data — two different claims, and neither is a completion claim.** `630` is the flagged-source-field total *of the published authoritative register* named above, and it stays quoted as such wherever the validator pins it. A fresh collation of current `main` reports **532** flagged fields, because PRs #29/#30/#32/#34 re-keyed fields to their witnesses and no new register has been published since; that run is recorded in [`.orchestrator/PHASE2_PLAN.md`](./.orchestrator/PHASE2_PLAN.md) §2 (2026-09-12, base `b3cd14f`) and in [`.orchestrator/WITNESS_INVENTORY_XSERIES.md`](./.orchestrator/WITNESS_INVENTORY_XSERIES.md) family totals. The drop buys **no status**: manifest states are unchanged at 1 / 32 / 2, `corpus.complete_documents` is still empty, and all five documents touched by the campaign remain `partial_or_failed_w1_collation` pending the post-remediation evidence pass. Reduced flags are not a finished corpus — that misreading is exactly what the five-ledger separation exists to prevent.
...
- [ ] **Editorial review queue** (re-derived 2026-09-12 from `.orchestrator/PHASE2_PLAN.md`, whose 76 consolidated rows carry a reproduce command each):
  ...
  - **Publish the post-remediation evidence pass**: re-run the collation over current `main` and commit a new dated register/report, so the authoritative flagged total can move honestly from the 2026-09-10 register's 630 to today's measured 532 and the five re-keyed/labelled documents can be re-adjudicated.
