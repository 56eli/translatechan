...
> - **Phase 2 — `12/17 complete`** · denominator: the 16 documents in `data/corpus/` measured against this phase's own target, *complete* canonical ingestion. `python3 -c "import json;m=json.load(open('data/project_metrics.json'))['corpus'];print(m['documents'],len(m['complete_documents']),m['excerpt_seed_documents'])"` → `16 11 5`: the re-keyed Gateless Gate (2026-09-21) plus the ten documents marked on 2026-09-22 by task 058 (`congronglu`, `chuandenglu_full`, `caoshan_benji`, `huangbo_fayao_full`, `mazu_guanglu_full`, `yunmen_guanglu_full`, `dongshan_yulu_full`, `zhaozhou_yulu_full`, `dahui_yulu_full`, `linji_yulu`) are `complete_selected_witness` — each `collated_to_claimed_witness`, 0 flagged, unit target equal to the units present, and re-verified as a verbatim contiguous tiling of its pinned witness region ([`sessions/P1_COMPLETE_MARKING_2026-09-22.md`](./sessions/P1_COMPLETE_MARKING_2026-09-22.md)); 5 are excerpt seeds (`zhengdao_ge`, `hanshan_poems`, `niutou_juezhu`, plus the two restored Tier2 seeds `guiyang_yulu` and `fayan_yulu` added on 2026-09-22 by task 059 — measured excerpt seeds, re-key attempts NOT landed, excluded from the completion marks). "Complete" is scoped to the pinned extraction rule (`tei:note` apparatus dropped) and to the region each `coverage_note` declares; front/back matter outside that region is disclosed, not claimed. The representation targets inside the box are a different claim from completion and were met before the marking: 48/48 *Wumenguan* cases, 100/100 *Congronglu* case records, 16/16 manifest items, 4092/4092 declared case locators (6 document-level seeds remain; the pre-purge figures 100/100 *Biyanlu*, 36/36 items, 248/248 locators described the 2026-09-13 tree).
...
> - **Phase 5 & 6 — `Planned`** · denominator: delivered units, of which there are none to count. The only active work in these phases is lineage verification, with batch 1 reviewed on 2026-09-21: 31 internal edges, **10** `exact_locator_verified` and **21** `traditional_link_pending_exact_locator`, plus 4 frontiers (`data/lineage/lineage_verification.json`).
>
> **Attribution-integrity milestone (historical, 2026-08-08)**: provenance policy v2.2 is live (`data/translations/provenance.json`, explicit Reader/Matrix badges ✅/⚠️/🤖); **2 verified corpus quotation slots across 2 texts + 2 verified Matrix entries** (the Wumenguan quotation set covered 48/48 represented cases). Every verified source resolves through `rights_manifest.json`; this English quotation layer is separate from the current W1 source-collation status and rights decisions.
...
> W1 evidence: **17 documents, 127 flagged source fields** (authoritative 2026-09-22 P2 Tier2 batch1 overlay: `sessions/COLLATION_REGISTER_2026-09-22_P1_BIYANLU.json` — the two purged Tier2 retelling keys restored as measured excerpt seeds (Guiyang 0 of 6, Fayan 1 of 11 source-content fields verbatim; re-key NOT landed) as documents 15 and 16, re-measuring the 14 entries of the 2026-09-21 combined Wumenguan/Linji overlay `sessions/COLLATION_REGISTER_2026-09-21_WUMENGUAN_LINJI.json` byte-identically — the re-keyed Gateless Gate T48n2005 as the 13th document and the re-keyed Record of Linji T47n1985 as the 14th, inheriting the 12 entries of the 2026-09-21 enthusiast fulls overlay `sessions/COLLATION_REGISTER_2026-09-21_ENTHUSIAST_100PCT.json` verbatim, which itself inherits the 38 entries of the 2026-09-20 Caoshan Benji overlay `sessions/COLLATION_REGISTER_2026-09-20_CAOSHAN_BENJI.json`, the 37 entries of the 2026-09-20 full-Jingde-Chuandeng-Lu overlay `sessions/COLLATION_REGISTER_2026-09-20_CHUANDENGLU_FULL.json`, the 36 entries of the 2026-09-20 correction overlay `sessions/COLLATION_REGISTER_2026-09-20_CORRECTION.json`, and the 35 entries of the 2026-09-10 correction register `sessions/COLLATION_REGISTER_2026-09-10_CORRECTION.json` plus the Congrong Lu reinstatement; historical 2026-09-09 register: `sessions/COLLATION_REGISTER_2026-09-09.json` with 34 documents and 622 flagged fields — the 637 figure in the 2026-09-09 report is **superseded**); 4,709 of 4,734 source-content fields collate to their claimed witness, and 3 documents have no collating source-content field at all (the two witness-unavailable seeds plus guiyang_yulu). Authoritative record: `sessions/COLLATION_REGISTER_2026-09-22_P1_BIYANLU.json` (2026-09-22); historical record: `sessions/COLLATION_REGISTER_2026-09-09.json` (2026-09-09).
>
> **630 is the register designation; 486 is a recorded measurement — two different claims, and neither is a completion claim.** `630` is the flagged-source-field total *of the published authoritative register* named above, and it stays quoted as such wherever the validator pins it. A fresh collation of `main` as it stood on 2026-09-21 reported **486** flagged fields and **5,177 of 5,368** source-content fields collating to their claimed witness (that re-run covered all 44 documents then present; the 2026-09-20 measurement over the then 38 documents had reported 2,591 of 2,782), because PRs #29/#30/#32/#34 re-keyed fields to their witnesses, the 2026-09-20 Congronglu reinstatement added 500 collating fields with no flags, and the 2026-09-21 full-witness ingests added 2,586 more with none; the earlier 532/691 figures were published as a dated measurement record — [`sessions/COLLATION_REGISTER_2026-09-12_POSTREMEDIATION.json`](./sessions/COLLATION_REGISTER_2026-09-12_POSTREMEDIATION.json) with its report [`sessions/COLLATION_W1_2026-09-12_POSTREMEDIATION.md`](./sessions/COLLATION_W1_2026-09-12_POSTREMEDIATION.md) (2026-09-12, base `faff161`, kind `w1-post-remediation-measurement`, **not designated authoritative**, per the owner's ruling) — and are this measurement's predecessor, not the current one. The drop bought **no status**: `corpus.complete_documents` is still empty, and the 2026-09-21 purge then removed the retelling documents. The Linji re-key overlay measures the 13 active documents at 4,557 of 4,566 source-content fields collating with 15 flagged entries; the campaign's own register recorded that own `reproduction.documents_with_changed_status` is 0. Reduced flags are not a finished corpus — that misreading is exactly what the five-ledger separation exists to prevent.
...
| `linji_yulu` | #32 | CBETA T47n1985 | *(superseded 2026-09-21 — task P2.9: the record was purged with the other retellings and rebuilt from scratch out of the pinned witness; see the Linji Yulu item in Phase 2)* **5** fields re-keyed (R-A); **3** NOT_FOUND 行錄 retellings kept as project text with `editorial_note` labels and no witness attribution (owner's 2026-09-11 per-field ruling); 2 MINOR untouched by policy | content flags **10 → 5**; flagged **84 → 79**; collating **79/89 → 84/89**; `zh_chars` 13993 → 14206 |
...
- [x] **Interactive Web Application (GitHub Pages)**:
  ...
  - Instant client-side search across **all 14 active texts and primary schema shapes**, including case pointers (accurate matching-unit counts, highlighting, and jump-to-anchor — [`sessions/AUDIT_archive_2026-08-08.md` §10](./sessions/AUDIT_archive_2026-08-08.md#10-2026-08-08--current-independent-audit-post-pr-3)).
...
- [x] **Core Foundational Corpus** (36 active files: 35 after the 2026-08-10 Congronglu quarantine, plus the 2026-09-20 reinstatement of Congronglu itself from the pinned T48n2004 witness; editorial completion status is explicit in the manifest):
  ...
  2. *Linji Yulu* (臨濟語錄 / T1985 — **107/107 sections represented and collated to the pinned T47n1985 witness** (task P2.9, 2026-09-21): the whole 16,366-CJK fascicle in 107 verbatim contiguous sections, 215 measured fields, 215 EXACT, 0 flagged; the PR #32 patch history is superseded on the historical chain)
...
- [x] **Data Contract & Release Guardrails**:
  ...
  - `data/canonical_locators.json` covers all 14 active documents and 4,092/4,092 case records (100 Congronglu + 1,274 Jingde Chuandeng Lu + 84 Caoshan Benji + the 2,586 units of the six 2026-09-21 full records + the 48 Wumenguan cases). Ten documents are case-level; four are document-level, and `linji_yulu` now carries **107 `lb`-anchored section locators** (`sessions/P2_LINJI_YULU_2026-09-21_locators.json`), so the queue's remaining unit-locator gap is the three excerpt seeds. The quarantined Congronglu locator claims were removed after authoritative T48n2004 headings disproved their case numbering/page claims; the reinstated document's 100 case locators are heading-line anchors read from the same pinned witness, and they reproduce the five anchors the 2026-08-10 containment table recorded. `data/editorial/traceability_queue.json` tracks the four document-granularity records.
...
- [ ] **Full-Text Ingestion Targets** (status re-derived 2026-09-12; the Wave 1 section above carries the per-document harness deltas):
  ...
  - [x] Linji Yulu — **re-keyed 2026-09-21 (task P2.9)**: the purged retelling was replaced by `scripts/segment_linji_yulu.py`, a from-scratch extraction of the pinned T47n1985 witness tiling the whole 16,366-CJK fascicle in **107 verbatim contiguous sections** (4 序 prefaces, the fascicle heading, 38 record sections, 勘辨 23, 行錄 21, close and printing colophon) with the concatenation asserted to equal the digest-verified reference character for character; `scripts/collate_corpus.py --doc linji_yulu` measures **215 fields, 215 EXACT, 0 flagged** (107/107 source-content fields) and the manifest carries `collated_to_claimed_witness` (human sign-off, interlinear `<g>` apparatus handling and any complete-selected-witness claim remain pending)
...
│   ├── canonical_locators.json # Document/case source-locator registry (4,092/4,092 case records; 4 document-level seeds)
│   ├── project_metrics.json    # Deterministic generated project counts (incl. the corpus.source_review aggregate)
│   ├── corpus/                 # 16 active structured source files — 11 complete selected witnesses (marked 2026-09-22, task 058) / 0 partial / 5 excerpt seeds (2 restored 2026-09-22, task 059)
│   ├── editorial/              # traceability_queue.json (4 source-locator reviews)
...
> **Measured status refresh 2026-09-21 (P2 authenticity labels + enthusiast full-witness ingests):** the corpus carries **47** provenance-note strings across four keys (`cbeta_note` 28, `coverage_note` 42, `editorial_note` 16, `recension_note` 14), **32 of 47** rendering beside a passage (the 42 `coverage_note` lines stay ledgered by exemption), **35** documents carrying at least one label — P2 added 14 `editorial_note` R-B labels and 2 `coverage_note` rewrites (87→113), enthusiast added 6 `cbeta_note` + 6 `coverage_note` (113→125); W1 register 38→44 docs, flagged total stays 630.
