...
try {
  ...
  for (const [group, label, count] of [['complete_selected_witness', 'Complete witnesses', 1], ['partial_selected_witness', 'Partial witnesses', 9], ['excerpt_seed', 'Excerpt seeds', 3]]) {
...
try {
  ...
  if (!filterInput) { failures++; console.log('❌ 4hh: #corpus-filter-input missing'); }
    ...
    if (restoredCount !== 13) {
      failures++; console.log(`❌ 4hh: clearing the filter should restore all 13 entries (got ${restoredCount})`);
    }
  }
