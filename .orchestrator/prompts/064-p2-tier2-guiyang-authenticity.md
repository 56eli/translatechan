...
- **Target branch:** `fix/tier2-guiyang-authenticity` ← **this exact name, fresh.**
...
```bash
git fetch --depth 50 origin +fix/tier2-guiyang-authenticity:refs/remotes/origin/_resume
git checkout -B fix/tier2-guiyang-authenticity refs/remotes/origin/_resume
```

If that fetch finds no remote ref, the branch is new:

```bash
git fetch --depth 1 origin +main:refs/remotes/origin/main && git checkout -B fix/tier2-guiyang-authenticity origin/main
```
...
```bash
git add -A && (git diff --cached --quiet || git commit -qm "chore: wip <sub-task>") && git push -qu origin fix/tier2-guiyang-authenticity
```
