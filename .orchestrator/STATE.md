...
**Pages Scope — 2026-09-19 (replaces 2026-09-14 website ruling):**
- github pages deployment and creation is outside of the scope of translatechan agents, unless specifically asked for
...
- Common qualities note — Phase5 prompts seq>=25 must contain light mental load, English first, not dense, comfortable to read, easy to navigate, piece meal, plain language, info section for every work and teacher
...
## 2026-09-18 — Phase 1 Batch 0: variant 36 rebuilt faithfully from the owner's zip (PR open, awaiting owner rating)

- Batch 0 per work order 2026-09-18-letter-002-36-distinct-rebuild.md item 5: variant 36 ("The Chan Room") rebuilt from `chan-buddhism-digital-library.zip` (293,147 B @ main 9af86f2) — the zip's visual system on the prototype's OWN structure (F7 cure), not assimilated into the shared skeleton: Reader as the prototype's detail grid (reading column + sticky dossier aside), hairline card grids in Lineage/Cases, native `<details>` disclosure dossiers and record folds everywhere, rust-border passage figures, cream/rust token system, own `tcr-` class namespace (16 classes).
- The rebuild is a marked pure-append block (`rebuild:36 begin/end`) at both file tails; the fleet-assimilated v36 block above it stays byte-untouched (grandfathered per work order item 5 until the owner rates the rebuild — the rebuild supersedes it at runtime via later declaration, and removal of the grandfathered bytes is the owner's later call). Removing the marked region reproduces main @ 9af86f2 byte-for-byte (verified for app.css and app.js); scopes 1-35 + base shell byte-untouched.
- `python3 scripts/pages scope out of scope 36` → PASS (floor min 0.72rem; style= 0; setProperty census 4; own weight 33.4 KB ≤ 40 KB; 0/112 shared bodies; isolation pure append, guards, shared teardown). Required text-integrity gates green: validate_data corpus35 slots1252 verified177 matrix21 locators148/148 flagged630; build deterministic app_data.js 1,693,251 B unchanged; preservation 0 unauthorized; review 138; website ruling PASS. Runtime sanity (jsdom, verification-only): all five rooms restructure through the real switcher mechanism, idempotent on re-apply, no inline styles, no runtime errors.
