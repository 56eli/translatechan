...
**Not release-ready: 6.6/10 and `repo_ready = fail`.** The product purpose, static pipeline, provenance disclosure, and visual direction are strong, but the Lineage directory mode remains semantically hidden, non-case source shapes are weakly validated, browser/accessibility evidence is optional, all quotation-rights decisions remain open, and operations gates are incomplete.
...
1. **P1 — Lineage directory:** `#lineage-content-target` retains `hidden`; the mode control changes inline display only, so the directory cannot reliably render.
2. **P1 — rights and citations:** all 14 rights records require human review; 3 verified quotations retain pending references.
3. **P1 — validation:** JSON Schema is not executed and has no root application; malformed/empty non-case units pass the Python validator.
4. **P1 — browser evidence:** Playwright exits zero when unavailable and is not in CI; no current screenshots, axe run, dark/tablet suite, or owner approval exists.
5. **P2 — accessibility/responsive:** pinyin and Lineage mode state, tooltip relationships, radio behavior, stale case labels, search announcements, and mobile Lineage grid behavior need correction.
6. **P2 — error handling:** fatal bundle/init CSS exists but no runtime boundary renders it.
7. **P2 — operations:** four mirrored paths are absent from CI diff coverage; action versions emit Node 20 deprecation warnings; branch protection remains unverified.
8. **P2 — editorial depth:** 33 document-level locator migrations, Biyanlu/Linji review, 30 lineage edges, and 34 profile reviews remain.
9. **Contained P0 — Congronglu:** active unreliable data/locators/scripts remain removed; reintroduction requires authoritative TEI, per-field provenance, negative fixtures, and human review.
...
|---|---|---|---|
| 2026-08-10 | `019fecb1` | [`sessions/AUDIT_RESPONSE_2026-08-10_019fecb1.md`](./sessions/AUDIT_RESPONSE_2026-08-10_019fecb1.md) | **Current:** full engineering/design/data/operations audit; documentation and scoreboard reconciled to 6.6/10 |
