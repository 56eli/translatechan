...
**P2-C delivered — renderer escaping consistency:** ~60 interpolation sites across the reader (headers, pinyin/speaker lines, case/section/dialogue/stanza/chapter/five-ranks/sample-record fields), lineage cards, gong'an index (incl. the `data-gongan-filter` attribute), lexicon cards, and the master dossier now consistently `escHtml()` all data-derived strings. Previously only the Matrix escaped rigorously; latent (not active) injection surface is closed. New smoke check **4y** poison-fixtures a master name, gong'an theme, glossary term, and case title with `<img src=x onerror=…>` and asserts all four views render it escaped only — efficacy-verified by temporarily reverting one escape and watching the check fail.
