...
- Source volume: **104,564 content CJK / 110,165 all-string CJK**.
...
- Bundle: `app_data.js` and `docs/` are regenerated by `scripts/build_data_bundle.py`, which prints the authoritative byte count at build time (no hand-typed figure here: an exact size quoted in prose goes stale the moment the corpus changes).
...
- Flagged fields after the campaign: the authoritative 2026-09-10 register holds **630**; a fresh collation of current `main` reports **532** (`COLLATION_REFS=<refs> python3 scripts/collate_corpus.py --require-verified-refs`, recorded in `.orchestrator/PHASE2_PLAN.md` §2) — the difference is the four merged re-keys — and only the post-remediation evidence pass may supersede the register.
