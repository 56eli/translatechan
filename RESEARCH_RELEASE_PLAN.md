...
- [ ] **1. Post-remediation evidence pass — half delivered (PR #41, 2026-09-12); the designation half is held for an owner ruling and this box stays unticked.** *Measurement half — done:* the collation re-run over current `main` is committed as `sessions/COLLATION_REGISTER_2026-09-12_POSTREMEDIATION.json` + `sessions/COLLATION_W1_2026-09-12_POSTREMEDIATION.md`, with its refs manifest and every reference digest verified (39 verified / 0 drift at CBETA XML P5 revision `dbdea41071e1e260ad84b72faefd4587333cf76d`), and the five re-keyed/labelled documents re-adjudicated: **532** flagged fields, **691/924** content fields collating, statuses **1 / 32 / 2**, and `documents_with_changed_status: 0` — the re-measurement moved no document's status. *Designation half — held:* the exit also requires `data/project_metrics.json` regenerated through the protected `--write-metrics` path and the gate-pinned figures in `README.md` / `AUDIT.md` / `HANDOFF.md` / `ROADMAP.md` re-pointed at the new record. That re-designates the authoritative evidence, and `scripts/w1_evidence.py` pins exactly one historical register plus one `w1-correction` overlay (`FIXED_METADATA`), so the pair cannot move without a deliberate, owner-ruled change to the evidence model; documentation cannot close this half. *Who:* agent-executable, owner-merged. **Until that ruling lands, `630` stays "the register" and `532` stays "today's data as measured" — and neither is a completion claim.**
