...
- **Disclosure presentation:** the corpus carries **54** provenance notes (`cbeta_note` 17, `recension_note` 14, `coverage_note` 15, `editorial_note` 8) across 28 documents; the reader renders the three passage-level keys at **17** shared-renderer sites — **39 of 54** note strings beside their passages — while the 15 `coverage_note` strings render in the dossier ledger under a recorded exemption. Implemented and gate-guarded by PR #40; no real-browser evidence exists, so it is not browser-verified. Blocker 2 below is ticked on that basis.
