...
- **Source-integrity evidence:** the authoritative W1 record is `sessions/COLLATION_REGISTER_2026-09-20_CORRECTION.json` — **36 documents, 630 flagged source fields**, evidence date 2026-09-20 — which inherits the 35 per-document entries of `sessions/COLLATION_REGISTER_2026-09-10_CORRECTION.json` verbatim and adds the Congronglu reinstatement (0 flagged), with the 2026-09-09 register retained as append-only historical evidence. **630 is the register's figure, not today's measurement:** a fresh collation of current `main` reports **486** flagged fields, the difference being the four merged re-keys plus the reinstated document. The drop is **not** a completion claim and changed no document's status. Field-level state: 1,093 of 1,424 source-content fields collate, 22 documents have no collating source-content field, and 492 `title_zh`/`name_zh` metadata fields are measured in a separate partition.
