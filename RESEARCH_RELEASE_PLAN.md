...
- **Disclosure presentation:** the corpus carries **49** provenance notes (`cbeta_note` 16, `recension_note` 14, `coverage_note` 11, `editorial_note` 8) across 26 documents; the reader renders the three passage-level keys at **17** shared-renderer sites — **38 of 49** note strings beside their passages — while the 11 `coverage_note` strings render in the dossier ledger under a recorded exemption. Implemented and gate-guarded by PR #40; no real-browser evidence exists, so it is not browser-verified. Blocker 2 below is ticked on that basis.
