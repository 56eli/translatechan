...
- **Source-integrity evidence:** the authoritative W1 record is `sessions/COLLATION_REGISTER_2026-09-21_ENTHUSIAST_100PCT.json` — **12 documents, 15 flagged source fields**, evidence date 2026-09-21 — which inherits the 38 per-document entries of `sessions/COLLATION_REGISTER_2026-09-20_CAOSHAN_BENJI.json` verbatim and adds the six full-witness records (2,586 content + 2,592 metadata fields, all EXACT, 0 flagged), with the 2026-09-09 register retained as append-only historical evidence. **630 is the register's figure, not today's measurement:** a fresh collation of current `main` reports **486** flagged fields (re-run 2026-09-21 over all 44 documents), the difference being the four merged re-keys plus the reinstated and newly ingested documents. The drop is **not** a completion claim and changed no existing document's status. Field-level state: 5,037 of 5,368 source-content fields collate in the register (a fresh collation of current `main` measures 5,177 of 5,368 with the same 486 flags), 22 documents have no collating source-content field, and 4,444 `title_zh`/`name_zh` metadata fields are measured in a separate partition.
...
> **Measured status refresh 2026-09-21 (PURGE retellings removed):** the corpus carries **21** provenance-note strings across four keys (`cbeta_note` 12, `coverage_note` 10, `editorial_note` 0, `recension_note` 0), **11 of 21** rendering beside a passage, **11** documents carrying at least one label. **12 documents, 15 flagged source fields**.
