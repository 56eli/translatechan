...
>
**Refresh 2026-09-20 (P2 core-yulu re-key attempt, task 048):** the corpus carries **113** provenance-note strings across four keys (`cbeta_note` 22, `coverage_note` 36, `editorial_note` 41, `recension_note` 14), **77 of 113** rendering beside a passage (the 36 `coverage_note` lines stay ledgered by exemption), **29** documents carrying at least one label (unchanged); release blocker 2's presentation half improves by the same mechanism (12 new rendered `editorial_note` labels beside the two P2 documents' passages), with no source text re-keyed — the measured R-A attempts failed the 80-percent threshold (0 of 6 fields EXACT in any of the 40 refs for dazhu; 0 of 6 in the claimed X68n1315 for nanquan), so the retellings are retained with the attempt recorded.
**Refresh 2026-09-20 (P2 core-yulu authenticity labels, task 047):** the corpus carries **101** provenance-note strings across four keys (`cbeta_note` 22, `coverage_note` 36, `editorial_note` 29, `recension_note` 14), **65 of 101** rendering beside a passage (the 36 `coverage_note` lines stay ledgered by exemption), **29** documents carrying at least one label (unchanged); release blocker 2's presentation half improves by the same mechanism (14 new rendered labels — 13 `editorial_note` + 1 `cbeta_note` — beside the two P2 documents' passages), with no source text changed (no re-key: no verbatim carrier in any of the 40 pinned refs).
**Refresh 2026-09-20 (Caoshan Benji ingest):** the corpus carries **87** provenance-note strings across four keys (`cbeta_note` 21, `coverage_note` 36, `editorial_note` 16, `recension_note` 14), **51 of 87** rendering beside a passage (the 36 `coverage_note` lines stay ledgered by exemption), **29** documents carrying at least one label; release blocker 2's presentation half is unchanged by the ingest (one rendered `cbeta_note` added, one exempted `coverage_note` added). The W1 authoritative record moved 37 → 38 documents with the flagged total unchanged at 630.
...
- **Corpus:** 36 active documents, **0 complete** — `corpus.complete_documents` is empty. Completion statuses are **5 `partial_selected_witness`** (`wumenguan`, `biyanlu_cases`, `linji_yulu`, `xinxin_ming`, `congronglu`) and **31 `excerpt_seed`**. W1 source-review states are **2 `collated_to_claimed_witness`** (`zhengdao_ge`, `congronglu`), **32 `partial_or_failed_w1_collation`**, **2 `witness_unavailable`** (`hanshan_poems`, `niutou_juezhu`).
...
- **Source-integrity evidence:** the authoritative W1 record is `sessions/COLLATION_REGISTER_2026-09-20_CAOSHAN_BENJI.json` — **38 documents, 630 flagged source fields**, evidence date 2026-09-20 — which inherits the 37 per-document entries of `sessions/COLLATION_REGISTER_2026-09-20_CHUANDENGLU_FULL.json` verbatim and adds the Caoshan Benji record (0 flagged), with the 2026-09-09 register retained as append-only historical evidence. **630 is the register's figure, not today's measurement:** a fresh collation of current `main` reports **486** flagged fields, the difference being the four merged re-keys plus the reinstated document. The drop is **not** a completion claim and changed no document's status. Field-level state: 2,451 of 2,782 source-content fields collate in the register (a fresh collation of current `main` measures 2,591 of 2,782 with the same 486 flags), 22 documents have no collating source-content field, and 1,852 `title_zh`/`name_zh` metadata fields are measured in a separate partition.
- **Traceability:** 248 active case records have case locators (48 Wumenguan + 100 Biyanlu + 100 Congronglu) but only 3 documents are case-level; 33 remain document-level seeds with limited Linji/Xinxin unit pilots (queue: 30 `needs_unit_locator`, 3 `in_review`). A case-number anchor is not proof that every nested source field was collated — the W1 harness measures the fields, and it is what found the gaps.
