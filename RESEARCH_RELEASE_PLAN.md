...
>
**Refresh 2026-09-21 (task 050, six enthusiast full-witness ingests):** the corpus carries **99** provenance-note strings across four keys (`cbeta_note` 27, `coverage_note` 42, `editorial_note` 16, `recension_note` 14), **57 of 99** rendering beside a passage (the 42 `coverage_note` lines stay ledgered by exemption), **35** documents carrying at least one label; release blocker 2's presentation half is unchanged by the ingests (six rendered `cbeta_note` lines added, six exempted `coverage_note` lines added). The W1 authoritative record moved 38 → 44 documents with the flagged total unchanged at 630.
...
- **Corpus:** 13 active documents, **0 complete** — `corpus.complete_documents` is empty. Completion statuses are **10 `partial_selected_witness`** (`congronglu`, `chuandenglu_full`, `caoshan_benji`, the six 2026-09-21 full-witness records, `linji_yulu`) and **3 `excerpt_seed`** (`zhengdao_ge`, `hanshan_poems`, `niutou_juezhu`). W1 source-review states are **11 `collated_to_claimed_witness`**, **0 `partial_or_failed_w1_collation`**, **2 `witness_unavailable`** (`hanshan_poems`, `niutou_juezhu`).
...
- **Source-integrity evidence:** the authoritative W1 record is `sessions/COLLATION_REGISTER_2026-09-21_LINJI.json` — **13 documents, 15 flagged source fields**, evidence date 2026-09-21 — which inherits the 12 per-document entries of `sessions/COLLATION_REGISTER_2026-09-21_ENTHUSIAST_100PCT.json` verbatim and adds the re-keyed Record of Linji (107 verbatim sections tiling the whole 16,366-CJK T47n1985 fascicle; 215 fields measured, 215 EXACT, 0 flagged), with the 2026-09-09 register retained as append-only historical evidence. **630 is the register’s designated figure, not today’s measurement:** the 2026-09-21 post-remediation pass measured **486** flagged fields over the 44 documents then present, the difference being the merged re-keys plus the reinstated and newly ingested documents; that figure is historical now, and the 2026-09-21 purge removed the retelling documents. The drop is **not** a completion claim: `corpus.complete_documents` is still empty. Field-level state: **4,557 of 4,566** source-content fields collate in the register, 2 documents have no collating source-content field, and 4,167 `title_zh`/`name_zh` metadata fields are measured in a separate partition.
- **Traceability:** 4,044/4,044 declared case records have case locators (100 Congronglu + 1,274 Jingde Chuandeng Lu + 84 Caoshan Benji + the 2,586 units of the six 2026-09-21 full records) across 9 case-level documents; 4 documents remain document-level and *Linji* now carries 107 `lb`-anchored section locators read from the pinned T47n1985 witness (the three excerpt seeds carry none). A case-number anchor is not proof that every nested source field was collated — the W1 harness measures the fields, and it is what found the gaps.
