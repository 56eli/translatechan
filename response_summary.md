...
## Status: ✅ Classical Chan Content Wave 1 & 2 (4 Complete Texts: Wumenguan, Biyanlu, Xinxin Ming, Platform Sutra + Linji Yulu + Congronglu + Zhaozhou + Huangbo) + Full Audit + UI/ARIA Polish + Robolation Terminology Rebrand

All quality gates pass on the final tree: `validate_data.py` (`corpus=36 | slots=1183 | verified=177 | matrix=21 | locators=157/157`), `smoke_test.mjs` (36 texts exercised, 0 crashes, 27 test sections including `4z`), `py_compile scripts/*.py`, and root↔`/docs` byte-identical mirror.

### Deliverables & Remediations This Session (committed + pushed to `arena/019fea62-translatechan`)
1. **Classical Chan Ingestion Waves 1 & 2 (`scripts/ingest_content_wave.py` & `scripts/ingest_linji_and_platform_sutra.py`)**:
   ...
   - **Record of Zhaozhou (`T1987`)**: Expanded from 3 dialogues to **8 signature encounter dialogues** (*The Great Death*, *Four Gates*, *Put It Down*, *Where Buddha Is Not*, and *Zhaozhou's Dog*).
   - **Huangbo Transmission of Mind (`T2012A`)**: Expanded from 1 section to **5 canonical sermons** (*Mind is Buddha*, *The Void*, *Instant Awakening*, and *Refusing to Seek Outside*).
...
7. **Documentation & Quality Gate Sync** — Synchronized `AUDIT.md`, `README.md`, `HANDOFF.md`, and `ROADMAP.md` with the new metrics (`corpus=36 | slots=1183 | verified=177 | matrix=21 | locators=157/157`, `103,682 source-content CJK characters`, `32 excerpt seeds`, `4 complete texts`). Re-bundled `app_data.js` and synced `/docs` mirror.

### Verified Project Metrics & Scope
- `corpus`: 36 documents (**4 complete texts**: `wumenguan` 48/48 complete, `biyanlu_cases` 100/100 complete, `xinxin_ming` 37/37 complete, `platform_sutra` 10/10 complete; `linji_yulu` 74 sections; `congronglu_cases` 9 cases; `zhaozhou_yulu` 8 dialogues; `huangbo_chuanxin` 5 sections; 32 excerpt seeds); **103,682 content CJK / 108,985 all-string CJK**.
- `translations`: 1,183 corpus slots; 177 verified quotations; 21 matrix registers.
...
### Next Directions
- **Content Phase 2 (Congronglu & Further Sayings):** Expand Congronglu from 9 cases toward all 100 cases, or continue ingesting recorded sayings of Zhaozhou, Huangbo, and Linji.
