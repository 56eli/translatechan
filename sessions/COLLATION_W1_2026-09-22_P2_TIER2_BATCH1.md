# W1 Correction Step — Guiyang + Fayan Restored as Measured Excerpt Seeds (2026-09-22)
...
The 2026-09-09 report/register, the 2026-09-10 correction overlay, and the 2026-09-20/21 overlays are **historical evidence and are append-only**. This file is a **new dated overlay**: the two Tier2 retelling keys the 2026-09-21 purge removed are restored to the corpus as measured excerpt seeds and recorded as documents 15 and 16. The overlay covers each of the 16 current manifest items.
...
| what is the authoritative W1 status of the 16 active items | **this** overlay + `sessions/COLLATION_REGISTER_2026-09-22_P2_TIER2_BATCH1.json` (16 documents, 41 flagged entries) |
...
| **41** | **16 documents** | `sessions/COLLATION_REGISTER_2026-09-22_P2_TIER2_BATCH1.json` | **authoritative.** 16 active documents; the 15 → 41 move is exactly the two restored records' 26 non-collating fields (guiyang 10, fayan 16). |
...
Verification results for the 22 works the collator actually reads:

| comparison | verified | drift | unlisted |
|---|---:|---:|---:|
| against `sessions/COLLATION_W1_2026-09-22_P2_TIER2_BATCH1_refs_manifest.txt` (22 works) | 22 | 0 | 0 |
| against `sessions/COLLATION_W1_2026-09-09_refs_manifest.txt` (187 works) | 18 | 4 | 0 |

Across all 187 manifest works of the historical 2026-09-09 digest manifest, the two references whose bytes moved against history are declared drift, never rescued: T47n1987B, T48n2001, T48n2004, X68n1315. The authoritative manifest itself is exactly the union of the witnesses and probes the 16 active manifest items claim (a 22-work subset of the committed 43-work task-056 measurement set, all digests byte-identical; the 43-work set re-verifies 43/0 drift against the committed task-056 manifest).
...
Derived from the merged evidence for all 16 manifest items by `source_review.derive_status()`:
...
| `partial_or_failed_w1_collation` | 2 | `guiyang_yulu` (0/6), `fayan_yulu` (1/11) |
| `witness_unavailable` | 2 | `hanshan_poems`, `niutou_juezhu` |

Field totals over the 16 entries: **8,970 total fields**, **4,734 content fields**, **4,709 collated content fields**, **4,236 metadata fields, 16 of them non-collating**.

`documents_classification_identical = 5 of 7`
...
* `sessions/COLLATION_REGISTER_2026-09-22_P2_TIER2_BATCH1.json` — sha256 `ac100e22c820ada6b8ea3fb1a9cc84a650a37580709a17ca4a3526ab877c90c6`
* `sessions/COLLATION_W1_2026-09-22_P2_TIER2_BATCH1_refs_manifest.txt` — sha256 `36bfb2af6ddf0bab7bcdae03c3f2fa93a0595aa419a58d0ee6704d37311b34e5`
