...
**Close-out addendum (auth-recovery reconciliation):** the sandbox snapshot that preceded this turn predated the remote branch's post-outage pushes; fetching revealed four additional session commits already on the remote — **Linji (T1985) completion pilot A–D** (prefaces + Ascending the Hall + 示眾 + 勘辨 divisions from CBETA P5 XML, 67 sections / 13,367 zh chars; slots 949→1023; CJK 101,198/106,160). The re-committed local delta and the remote tip were merged; conflicts were confined to four prose docs (union-resolved) and four regenerated artifacts (rebuilt deterministically). All gates re-verified green on the merged tree (bundle 1,685,556 bytes).
