...
> **Where the corpus actually stands (2026-09-21).** **13 documents**, **0 complete**, **3 excerpt seeds**, **767,450** source-content CJK characters (**824,896** across every corpus JSON string) — `python3 -c "import json;m=json.load(open('data/project_metrics.json'))['corpus'];print(m['documents'],m['complete_documents'],m['excerpt_seed_documents'],m['content_cjk_characters'],m['all_corpus_cjk_characters'])"`. W1 source-review states: **11** `collated_to_claimed_witness` (`zhengdao_ge`, `congronglu`, `chuandenglu_full`, `caoshan_benji`, the six 2026-09-21 full-witness records and the re-keyed Record of Linji), **0** `partial_or_failed_w1_collation`, **2** `witness_unavailable`. The Wave 1 integrity campaign (PRs #29, #30, #32, #34, #35) re-keyed four documents to their pinned CBETA witnesses and labelled a fifth; three independent witness inventories plus [`.orchestrator/PHASE2_PLAN.md`](./.orchestrator/PHASE2_PLAN.md) now covered 36 documents before the 2026-09-21 purge, and [`ROADMAP.md`](./ROADMAP.md) records the campaign. §1.1 below states, objective by objective and element by element, how far each ambition is from being met. **Nothing here is softened**: the objectives keep their full strength, *Congronglu* is reinstated and fully collated (§2 note) without human sign-off, and no reduction in flagged fields is presented as completion.
