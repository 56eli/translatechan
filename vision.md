...
- [x] Surface every provenance label the data carries (`cbeta_note`, `editorial_note`, `recension_note`) beside the passage it describes — PR #40 renders **39 of the corpus's 50 note strings** at 17 shared-renderer call sites, one line each; the other 11 are `coverage_note` ledger strings under the single recorded exemption. *(Implemented and gate-guarded; no real-browser evidence exists, so it is not browser-verified — §1.1.)*
