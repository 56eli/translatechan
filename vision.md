...
> **Document type**: *aspirational architecture with a current public-scope note*, aligned to the measured status quo **as of 2026-09-21 (task 050 enthusiast fulls refresh)**. Prose blocks below carry their own dated measurements; current gate-run totals are **47** provenance-note strings across four keys (`cbeta_note` 27, `recension_note` 14, `coverage_note` 42, `editorial_note` 16) with **57 of 99** passage-level strings rendered across **35** documents. For **measured current status**, see [`AUDIT.md`](./AUDIT.md); for phase tracking and each phase's denominator, see [`ROADMAP.md`](./ROADMAP.md); for release gating, see [`RESEARCH_RELEASE_PLAN.md`](./RESEARCH_RELEASE_PLAN.md); for operational flow, see [`HANDOFF.md`](./HANDOFF.md). The public Pages interface is deliberately limited to Reader, Matrix, Lineage, Gong'an Index, and Lexicon; it does not expose browser drafting, Arena-agent branding, or a header GitHub link.
...
> **Where the corpus actually stands (2026-09-22).** **16 documents**, **11 complete** (`complete_selected_witness`: the re-keyed Gateless Gate plus the ten documents marked on 2026-09-22 by task 058 — `congronglu`, `chuandenglu_full`, `caoshan_benji`, `huangbo_fayao_full`, `mazu_guanglu_full`, `yunmen_guanglu_full`, `dongshan_yulu_full`, `zhaozhou_yulu_full`, `dahui_yulu_full`, `linji_yulu` — each collated with 0 flagged fields and re-verified as a verbatim contiguous tiling of its pinned witness region, [`sessions/P1_COMPLETE_MARKING_2026-09-22.md`](./sessions/P1_COMPLETE_MARKING_2026-09-22.md)), **5 excerpt seeds** (`zhengdao_ge`, `hanshan_poems`, `niutou_juezhu`, plus `guiyang_yulu` and `fayan_yulu` — the two purged Tier2 keys task 059 restored on 2026-09-22 as measured excerpt seeds, re-key attempts measured and NOT landed), **775,562** source-content CJK characters (**833,842** across every corpus JSON string) — `python3 -c "import json;m=json.load(open('data/project_metrics.json'))['corpus'];print(m['documents'],len(m['complete_documents']),m['excerpt_seed_documents'],m['content_cjk_characters'],m['all_corpus_cjk_characters'])"` → `16 11 5 775562 833842`. W1 source-review states: **12** `collated_to_claimed_witness` (`zhengdao_ge`, `wumenguan`, `congronglu`, `chuandenglu_full`, `caoshan_benji`, the six 2026-09-21 full-witness records and the re-keyed Record of Linji), **2** `partial_or_failed_w1_collation` (`guiyang_yulu`, `fayan_yulu`), **2** `witness_unavailable`. "Complete" is scoped to the pinned extraction rule `cbeta-p5-body-cjk-v1` and to the witness region each document's `coverage_note` declares; interlinear apparatus and disclosed front/back matter are not claimed. The Wave 1 integrity campaign (PRs #29, #30, #32, #34, #35) re-keyed four documents to their pinned CBETA witnesses and labelled a fifth; three independent witness inventories plus [`.orchestrator/PHASE2_PLAN.md`](./.orchestrator/PHASE2_PLAN.md) now covered 36 documents before the 2026-09-21 purge, and [`ROADMAP.md`](./ROADMAP.md) records the campaign. §1.1 below states, objective by objective and element by element, how far each ambition is from being met. **Nothing here is softened**: the objectives keep their full strength, *Congronglu* is reinstated and fully collated (§2 note) without human sign-off, and no reduction in flagged fields is presented as completion.
...
1. **Exhaustive Canonical Ingestion — much closer on 2026-09-22, still not met.** 16 documents are ingested (`ls data/corpus/*.json | wc -l` → 16) out of a stated scope spanning Taishō 47/48/51 and the *Zokuzōkyō*; **11 are complete** (`corpus.complete_documents` lists 11 keys after the 2026-09-22 complete marking, task 058 — complete meaning the pinned witness region tiles verbatim under `cbeta-p5-body-cjk-v1`, not that every recension's front matter or apparatus is carried) and **5 are excerpt seeds** (the three pre-existing plus the two restored Tier2 seeds); the objective's distance is now the un-ingested titles, not the completeness of the ingested ones. 4,709 of 4,734 source-content fields (99.5%) collate to their claimed witness, and 3 documents have no collating source-content field at all (`data/project_metrics.json` → `corpus.source_review`). The full-record gaps this phase named are now closed for Huangbo's 傳心法要, Mazu's X69n1321 record, the Yunmen Guanglu, both Dongshan fascicle parts, Zhaozhou's Guzunsu-yulu span and the Dahui Yulu including its letters, plus the full *Jingde Chuandenglu* and the Caoshan record; what remains un-ingested includes the monastic codes as complete texts, Hongzhi's Guanglu as its own record, the other Guzunsu masters' juan, and every recension's front matter and interlinear apparatus. *Congronglu* is reinstated with **100/100** cases collating, but its front matter, interlinear 著語 apparatus and human sign-off are not (§2 note). Ingestion is deterministic but manual per document — `scripts/segment_full_witness.py` and `scripts/segment_caoshan_benji.py` produce pinned-witness full records with tiling assertions; `scripts/segment_classical.py` remains an offline manual segmenter, `scripts/ingest_cbeta.py` a deprecated wrapper, and no end-to-end automated corpus pipeline exists; the W1 harness (`scripts/collate_refs.py`) extracts pinned witnesses for collation.
...
| :--- | :--- | :--- |
| **primary-source locators** | **Partial.** `data/canonical_locators.json` covers all 13 active documents and 4,044/4,044 declared case records (100 *Congronglu* + 1,274 *Chuandenglu* + 84 *Caoshan* + the 2,586 units of the 2026-09-21 full records) — **9** documents are case-level and **4** remain document-level, and the editorial queue holds four `complete` records. *Linji* now carries 107 `lb`-anchored section locators read from the pinned T47n1985 witness; page/line or TEI anchors for the three excerpt seeds do not exist. A case-number anchor is not proof that every nested source field was collated. | `python3 -c "import json;print(json.load(open('data/project_metrics.json'))['canonical_locator_coverage'])"` |
...
- **Census refresh 2026-09-21 (P2 authenticity labels + enthusiast fulls):** the corpus carries **47** provenance-note strings across four keys (`cbeta_note` 28, `coverage_note` 42, `editorial_note` 16, `recension_note` 14), **32 of 47** rendering beside a passage (the 42 `coverage_note` lines stay ledgered by exemption), **35** documents carrying at least one label.
...
> **14 documents, 15 flagged source fields** — combined Wumenguan/Linji overlay, 14 docs remain (12 collated +2 unavailable), flagged total 15
