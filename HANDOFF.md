...
- Bundle <2 MB: raw 1,925,366 B (~1.84 MB), gzipped 586,529 B — measured `gzip -c app_data.js app.js app.css index.html | wc -c` on main 3a6ae32, ceiling 2 MB tracked after Phase 3. **Testing phase ceiling extended to 30MB per RULING_BUNDLE_CEILING_2026-09-14.md (owner 2026-09-14) to enable drastic layout changes across ALL rooms — technically feasible, Quality workflow doesn't enforce size directly, presentation gates allowed to break while experimenting per RULING_GATES_EXPERIMENT_2026-09-14.md**. The full data bundle initializes up front; since Phase 2 the hidden rooms defer *rendering* only (first tab activation, per Checkpoint-C C-4 — option B bundle-splitting was not taken and remains open if browser measurements justify it). **PR-D perf folded into Phase 2 lazy**.
