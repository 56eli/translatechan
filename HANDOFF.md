...
```text
corpus=13 | slots=46 | verified=1 | matrix=21 | locators=4044/4044
content CJK=767,450 | all-string CJK=824,896
source-review: collated=11 | partial/failed=0 | unavailable=2
w1-evidence: active flagged=15 (authoritative 2026-09-21 Linji re-key overlay, 13 documents) | register designation=630 (owner ruling 2026-09-12) | historical=622 (2026-09-09) | report-figure-superseded=637
...
gates: py_compile OK; validate_data.py PASS (full + --write-metrics); build_data_bundle.py deterministic (run twice, identical bytes; data/ mirror byte-identical). RED (pre-existing, verified in a clean clone of the committed commit f1207ea *before* any P2.9 edit): scripts/test_source_preservation.py (FileNotFoundError data/corpus/wumenguan.json — base-commit comparison has no purge declaration), scripts/test_source_review_rules.py (KeyError: 'wumenguan' — the mutation matrix still keys a purged document and re-types 44-era totals), scripts/smoke_test.mjs (44-item manifest pin; the 13-item repin is a separate pass — see docs/STUB_P2.10_GATE_SUITE_REPIN.md). This re-key repins only the smoke suite's W1-evidence pointers/counts and its Linji expectations (Section source: T47n1985_p0504a26–p0504a29 for sections.four_shouts, collated_with_normalization, 赤肉團 search hit — all three verified with a DOM-stub run) — register **not** superseded: owner ruled 2026-09-12 that the post-remediation evidence pass (PR #41) does not replace it, so 630 stays the register designation while the active 13-document corpus carries 15
complete=0 | partial=10 | excerpt seeds=3
lineage=35 masters / 31 edges | glossary=31 | gong'an=24
app_data.js raw size as printed by scripts/build_data_bundle.py at build time / 13 documents
...
Verified citation reference coverage is **2 / 3**; the remaining **3** references are explicitly pending. Edition verification still does not establish reuse rights.
