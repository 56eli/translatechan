...
├── data/
│   ├── corpus_manifest.json    # Shared active-corpus order/navigation manifest (44 keys)
...
> **Measured status refresh 2026-09-21 (P2 Tier2 remaining follow-up):** the corpus carries **184** provenance-note strings across four keys (`cbeta_note` 30, `coverage_note` 42, `editorial_note` 98, `recension_note` 14), **142 of 184** rendering beside a passage (the 42 `coverage_note` lines stay ledgered by exemption), **37** documents carrying at least one rendered label — this follow-up added 34 note strings across six documents: 6 root `editorial_note` lines (the 054-pattern "Project retelling — no witness attribution" disclosure), 27 dialogue `editorial_note` R-B labels (6 `deshan_yulu`, 5 `xuansha_yulu`, 4 `xuefeng_yantou`, 2 `yuanwu_letters`, 6 `foyan_qingyuan`, 4 `caoxi_zhuan`), and the additive `yuanwu_letters` `cbeta_note` (150 → 184); it also rewrote 6 `coverage_note` lines to the full per-field run measurement on the 43-work pinned set (43 verified / 0 drift) and extended 5 `cbeta_note` lines — no re-key landed (every document measures below the 80 percent re-key threshold; the seventh Tier2 work, `sengzhao_zhaolun`, was confirmed 4/4 EXACT in its claimed T45n1858 and stays unlabelled). The W1 register's authoritative flagged total stays 630.
